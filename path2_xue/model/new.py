...
def get_args():
    ...
    parser.add_argument('--learning_rate', '-l', metavar='LR', type=float, default=1e-5, help='Learning rate',
                        dest='lr')
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_loader.dataset), desc=f'Epoch {epoch + 1}/{epochs}', unit='image') as pbar:
            for batch in train_loader:
                ...
                outputs_train.extend(outputs.detach().cpu().numpy())
                predicted_labels_train.extend(predicted_labels.detach().cpu().numpy())
